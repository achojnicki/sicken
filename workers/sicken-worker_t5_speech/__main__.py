...
class Worker_T5_Chat:
	project_name='sicken-worker_t5_chat'

	def __init__(self):
		self.config=adisconfig('/opt/adistools/configs/sicken-worker_t5_speech.yaml')
		self.log=Log(
            parent=self,
		...
		self.log.info('Initialisation of sicken-worker_t5_speech started')
		self._init_mongo()
		self._init_rabbitmq()
		self._init_model()
		self._init_tokenizer()
		self.log.success('Initialisation of sicken-worker_t5_speech succeed')
	...
	def _init_rabbitmq(self):
		...
		self._rabbitmq_channel.basic_consume(
			queue="sicken-requests_t5_spoken",
			auto_ack=True,
	...
	def _callback(self, channel, method, properties, body):
		...
		response=self._get_answer(msg)



		self._rabbitmq_channel.basic_publish(
			exchange="",
			routing_key="sicken-responses_spoken",
			body=response)
